#!/usr/bin/env python3
"""
GitMine TTY — Relay WebSocket Bridge

Self-hosted WebSocket broadcaster that:
...
Usage:
  ...
  # or as a systemd service (see relay/gitmine-relay.service)

Clients connect to:
  ws://<your-ip>:8765/ws

Webhook endpoint (configure in GitHub repo settings):
  http://<your-ip>:8766/webhook
"""
...
class RelayBridge:
    def __init__(self, config: dict):
        ...
        self.host = config.get("operator", {}).get("relay_host", "0.0.0.0")
        self.port = config.get("operator", {}).get("relay_port", 8765)
        self.stats = {
            ...
            async for message in ws:
                try:
                    ...
                    if data.get("type") == "ping":
                        ...
                        await ws.send(json.dumps({"type": "status", "relay": self.stats}))
    ...
    async def health_handler(self, request: web.Request) -> web.Response:
        ...
        return web.json_response({
            "status": "ok",
            "bridge": "gitmine-relay",
            "version": "1.0.0",
            **self.stats
        })

    async def run(self):
        log.info(f"Relay Bridge starting on {self.host}:{self.port}")
        ...
        log.info("Relay Bridge ready. Waiting for connections...")
...
def main():
    parser = argparse.ArgumentParser(description="GitMine TTY Relay WebSocket Bridge")
    ...
    if args.port:
        config["operator"]["relay_port"] = args.port
    if args.host:
        config["operator"]["relay_host"] = args.host

    bridge = RelayBridge(config)
