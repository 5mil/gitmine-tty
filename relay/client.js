/**
 * GitMine TTY — Relay WebSocket Client
 * Isomorphic: works in browser (PWA dashboard) and Node.js (TUI command)
 *
 * Usage (browser):
 *   import { RelayClient } from './relay/client.js';
 *   const relay = new RelayClient('ws://192.168.1.100:8765');
 *   relay.on('state_update', (msg) => updateDashboard(msg));
 *   relay.on('share_submitted', (msg) => flashShareCounter(msg));
 *   relay.connect();
 *
 * Usage (Node.js / TUI):
 *   const { RelayClient } = require('./relay/client.js');
 *   const relay = new RelayClient(process.env.RELAY_URL);
 *   relay.on('heartbeat', ({ rig_id }) => markRigOnline(rig_id));
 *   relay.connect();
 */
...
export class RelayClient {
  ...
  connect() {
    this._intentionalClose = false;
    this._log(`Connecting to Relay at ${this.url}`);
  ...
  _log(...args) {
    if (this.options.debug) console.log('[RelayClient]', ...args);
  }
}
