...
async fn handle(stream: TcpStream, addr: String, shared: Arc<Shared>) -> Result<()> {
    ...
    // Subscribe to new job broadcasts
    let mut job_rx = shared.jobs.subscribe().await;

    loop {
        tokio::select! {
            // New job from job manager → push mining.notify
            job = job_rx.recv() => {
            ...
            n = reader.read_line(&mut line) => {
                ...
                let response = dispatch(&req, &mut sess, &shared, &addr).await;
                ...
                // After authorize succeeds, immediately send current jobs
...
async fn dispatch(
    ...
    match req.method.as_str() {
        ...
        "mining.authorize" => {
            ...
            // Persist worker to DB (non-blocking)
        ...
        "mining.submit" => {
            ...
            let result = shares::validate(&job, en1, &p.extranonce2, &p.ntime, &p.nonce, cur_diff);

            let (accepted, share_row) = match result {
                Ok(s) => {
                    if s.is_block {
                        tracing::info!("[{}] *** BLOCK FOUND *** coin={} height={}", addr, coin, job.height);
                    } else {
            ...
            if let Some(new_diff) = shared.vardiff.on_share(worker) {
                tracing::debug!("[{}] vardiff → {}", addr, new_diff);
                // We can't send here directly (no write handle), so we'd need to
                // signal via a channel. For now log; full impl in next iteration.
            }
...
/// Generate a unique 8-byte extranonce1 hex string.
fn gen_extranonce1() -> String {
    let bytes: [u8; 4] = rand::thread_rng().gen();
    hex::encode(bytes)
}

/// Detect coin from Stratum password field: "FNNC", "fnnc", or "d=N" defaults to TTY.
fn coin_from_password(pass: &str) -> String {
    let upper = pass.to_uppercase();
    if upper.contains("FNNC") {
        "FNNC".to_string()
    } else {
        "TTY".to_string()
    }
